  ...
  <div>
    <v-form ref="basicInfoForm" v-model="valid">
      <div class="form-post-job">
        ...
        <v-row class="form-group" v-for="(address, index) in addresses" :key="index">
          <v-col md="5" cols="12">
            <v-row class="d-flex align-items-baseline">
              <v-col md="3" cols="12">
...
export default {
  ...
  watch: {
    ...
    job_post() {
      let x;
      this.job_name = this.job_post.job_title
       for(x=0;x<this.job_post.addresses.length;x++) {
          this.addresses[x] = {
            city: this.job_post.addresses[x].city_id,
            area: this.job_post.addresses[x].area_id,
            areas: []
          }
          this.cityUpdateSelect(x);
        }
      this.category = this.job_post.category_id
      this.employees_number = this.job_post.vacancies_no
      this.shift_type = this.job_post.shift_type_id
      this.job_details = this.job_post.description
