...
export default {
  ...
  watch: {
    ...
    job_post() {
      if (this.job_post.optional_requirements) {
        var op_req = JSON.parse(JSON.stringify(this.job_post.optional_requirements));
        if (op_req) {
          var optional_requirements = Object.keys(op_req).map((key) => [key, op_req[key]]);
          for (let x = 0; x < this.ORs.length; x++) {
            for (let i= 0; i < optional_requirements.length; i++) {
              let title = optional_requirements[i][0].split(' :')[0] ? optional_requirements[i][0].split(' :')[0] : '';
              if (this.ORs[x].title_ar == title) {
                this.optional_requirements_labels[x] = this.ORs[x].title_ar
                this.optional_requirements_values[x] = optional_requirements[i][1]
              }
            }
          }
        }
      }
      if (this.job_post.job_benefits) {
        var jo_ben = JSON.parse(JSON.stringify(this.job_post.job_benefits));
        if (jo_ben) {
          var job_benefits = Object.keys(jo_ben).map((key) => [key, jo_ben[key]]);
          for (let x = 0; x < this.JBs.length; x++) {
            for (let i= 0; i < job_benefits.length; i++) {
              let title = job_benefits[i][0].split(' :')[0] ? job_benefits[i][0].split(' :')[0] : '';
              if (this.JBs[x].name_ar == title) {
                this.job_benefits_labels[x] = this.JBs[x].name_ar
                this.job_benefits_values[x] = job_benefits[i][1]
              }
            }
          }
        }
      }
      this.required_gender = this.job_post.required_gender_id ? this.job_post.required_gender_id.toString() : ''
      this.is_disability = this.job_post.is_disability
      this.disability = this.job_post.disability
    }
  },
  async mounted() {
    this.getOR();
