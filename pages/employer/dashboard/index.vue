  ...
  <div>
    ...
    <div v-if="!$device.isMobile">
      <div class="container employer-dashboard">
        <v-dialog v-if="$device.isTablet" v-model="notification_dialog" max-width="600">
          <v-card light>
        ...
        <v-row v-if="$device.isTablet">
          <div class="ml-4 mr-auto">
            <v-btn color="#f3922a" dark rounded @click="notification_dialog=true">
                  ...
                  <v-col md="4" class="job-details-title text-left">
                    <span class="text-left">
                      <router-link
                        ...
                        :to="`/employer/${employer.slug}`"
                      >عرض صفحة الشركة ></router-link>
                    </span>
                  </v-col>
                  <v-col md="1" class="job-details-title text-left">
                    <span class="text-left">
        ...
        <v-row>
          <v-col md="3" v-if="!$vuetify.breakpoint.smAndDown">
            <div class="support-team pr-4 pl-4">
              <div class="job-title text-center">فريقنا هنا للمساعدة</div>
              <v-divider></v-divider>
              <div v-if="account_manager">
                <v-row>
                  <v-col md="12" class="ml-auto mr-auto d-block text-center">
                    <span class="job-details-title">تواصل مع مدير حسابك</span>
                  </v-col>
                </v-row>
                ...
                <v-row>
                  <v-col
                    ...
                    md="12"
                    class="ml-auto mr-auto d-block text-center"
                  >
                    <v-avatar class="ml-auto mr-auto" color="grey" size="164" tile>
                      <v-img max-width="164" :src="storage_url+account_manager.avatar"></v-img>
                    </v-avatar>
                  </v-col>
                  <v-col
                    v-if="$device.isTablet"
                    ...
                    class="ml-auto mr-auto d-block text-center"
                  >
                    <v-avatar class="ml-auto mr-auto" color="grey" size="100" tile>
                      <v-img
                        :aspect-ratio="0"
                        max-width="100"
                        :src="storage_url+account_manager.avatar"
                      ></v-img>
                    </v-avatar>
                  </v-col>
                </v-row>
                ...
                <v-row>
                  <v-col md="12" class="text-center">
                    <span class="job-title">{{ account_manager.name }}</span>
                  </v-col>
                </v-row>

                <v-row>
                  <v-col md="12" class="text-center">
                    <span class="job-title">{{ account_manager.phone_number }}</span>
                  </v-col>
                </v-row>

                <v-row>
                  <v-col md="12" class="text-center pl-0 pr-0">
                    <span class="job-title">{{ account_manager.email }}</span>
                  </v-col>
                </v-row>
              </div>
              <v-divider></v-divider>
              <v-row v-if="rec_specialists && rec_specialists.length > 0">
                <v-col md="12" class="ml-auto mr-auto d-block text-center">
                  <span class="job-details-title">تواصل مع مسئولين التوظيف</span>
                </v-col>
                <div v-for="(rec_specialist,index) in rec_specialists" :key="index">
                  <v-col md="12" class="ml-auto mr-auto d-block text-center">
                    <v-img max-height="135" :src="storage_url+rec_specialist.avatar"></v-img>
                  </v-col>

                  <v-row>
                    <v-col md="2" class="job-items mr-0 text-center">
                      <img src="~/assets/img/icons/cer..svg" />
                    </v-col>
                    <v-col md="10" class="text-center">
                      <span class="job-title">{{ rec_specialist.name }}</span>
                    </v-col>
                  </v-row>
                  ...
                  <v-row>
                    ...
                    <v-col md="10" class="text-center">
                      <span class="job-title">{{ rec_specialist.phone_number }}</span>
                    </v-col>
                  </v-row>
                  ...
                  <v-row>
                    ...
                    <v-col md="10" class="text-center">
                      <span class="job-title">{{ rec_specialist.email }}</span>
                    </v-col>
                  </v-row>
                </div>
              </v-row>
          ...
          <v-col md="9" sm="12">
            <div class="dashboard-job-candidates">
              <v-row>
                ...
                <v-col md="4" class="job-title text-left">
              ...
              <v-row v-if="subscription">
                <v-col md="6">
                  <label class="job-title" style="float: right">أيام حتى تجديد</label>
                  <label class="label-title">{{ subscription.days_left + 'يوم'}}</label>
                  <v-progress-linear
                    height="6"
                    :value="days_left"
                    :color="getProgressColor(days_left)"
                  ></v-progress-linear>
                </v-col>
                <v-col md="6">
                  <label class="job-title" style="float: right">وظائف متبقية</label>
              ...
              <v-row v-if="subscription">
                <v-col md="6" v-if="subscription.package_details">
                  <label class="job-title" style="float: right">باحثين عن العمل متبقيين</label>
                  <label
                    class="label-title"
              ...
              <v-row v-if="!subscription" class="mt-6">
                ...
                <v-col md="4" class="text-right">
                  ...
                  >
                    <img src="~/assets/img/icons/add-package2.svg" width="125" />
                  </div>
                </v-col>
              </v-row>
            </div>
            <div class="dashboard-job-candidates">
              <v-row>
                ...
                <v-col md="4" class="job-details-title text-left">
                  <nuxt-link :to="'/employer/jobs/'+employer.slug">
                  ...
                  <div class="job-details">
                    ...
                    <v-row class="job-footer">
                      <v-col md="4">
                        ...
                        >
                          <span style="color: #0e4168">المتقدمين الجدد</span>
                        </v-btn>
                      </v-col>
                      <v-col md="4">
                        <div
                          class="candidate-details"
                        >{{ job.invited_job_seekers.length }} المتقدمين المقترحين</div>
                      </v-col>
                      <v-col md="4">
                        <div class="candidate-details">
                          {{
                    ...
                    <v-row>
                      <v-col md="4" class="ml-auto mr-auto">
                        <v-btn
                          class="job-submit"
                          type="button"
                          @click="$router.push('/employer/jobs/details/'+job.slug)"
    ...
    <v-row>
      <v-btn
        ...
        class="job-submit pr-2 pl-2"
        style="width: auto !important"
        @click="speeddial=true"
        color="#f3922a"
        dark
        fixed
        bottom
        right
      >
        <v-icon>mdi-account-circle</v-icon>مدير الحساب
      </v-btn>
    </v-row>
    ...
    <v-row justify="center">
      <v-dialog v-model="speeddial" max-width="290">
        <v-card>
          <v-col cols="12">
            <div class="support-team">
              <div class="job-title">فريقنا هنا للمساعدة</div>
              <v-divider></v-divider>
              <v-row v-if="account_manager">
                <v-col cols="12" class="ml-auto mr-auto d-block text-center">
                  <span class="job-details-title">تواصل مع مدير حسابك</span>
                </v-col>
                <v-col cols="12" class="ml-auto mr-auto d-block text-center">
                  <v-img max-height="135" :src="storage_url+account_manager.avatar"></v-img>
                </v-col>
                <!-- <v-row> -->
                <!-- <v-col md="2" class="job-items mr-0 text-center">
                    <img src="~/assets/img/icons/cer..svg" />
